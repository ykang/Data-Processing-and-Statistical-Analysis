...
output:
  slidy_presentation:
    footer: "Lecture 7: Supervised methods"
...
- Decision trees are useful when input vari- ables interact with the output in “if-then” kinds of ways (such as IF age > 65, THEN has.health.insurance=T). 
...
datasetTrain <- read.csv("train.csv", header=TRUE, sep = ",")
datasetTest <- read.csv("test.csv", header=TRUE, sep = ",")
```
...
barplot(table(datasetTrain$Survived,
              datasetTrain$Sex))
